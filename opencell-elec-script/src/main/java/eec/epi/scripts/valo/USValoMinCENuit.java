 ...
 */
public class USValoMinCENuit extends RatingScript {
    ...
    protected static String NUIT = "NUIT";
    protected static String COSPHI = "COSPHI";
    ...
    private Map<String, BigDecimal> applyAccordingToSubscription(RatingContext ratingContext,List<Map<String, Object>> list,Subscription subscription, ServiceInstance prod, WalletOperation walletOperation) {
        ...
        List<Map<String, Object>> filteredConso = conso.stream()
                .filter(map -> {
                    ...
                    return EA.equals(natureMesure) && NUIT.equals(posteHorosaisonier)&& OPENCELL.equals(origin) && VALORISE.equals(statut);
                })
                .collect(Collectors.toList());
        if (filteredConso.size() != 1)
            throw new BusinessApiException("one ct_conso EA nuit should be found ERROR subscription :" + subscriptionCode);
        ...
        if(Objects.equals(cf_nature_value, "BT")){
            price=ratingContext.getValueFromList(PX_EA_NUIT, ratingContext.getOFFER(), "BT", cf_code_exploitation, list, subscriptionCode);
        }else if(Objects.equals(cf_nature_value, "HTA")) {
            price=ratingContext.getValueFromList(PX_EA_NUIT, ratingContext.getOFFER(), "HTA", cf_code_exploitation, list, subscriptionCode);
        }
...
//        BigDecimal quantity = price.multiply(edr.getQuantity());
        BigDecimal unitPriceWithoutTax = ((new BigDecimal(String.valueOf(quantityConsoCOSPHI)).subtract(new BigDecimal("0.90")))
                .divide(new BigDecimal("5"), new MathContext(10, RoundingMode.HALF_UP))).multiply(new BigDecimal("-1"));
